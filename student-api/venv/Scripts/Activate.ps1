...
# Add Chocolatey path to PATH (if it's not already there)
$chocoPath = "C:\ProgramData\chocolatey\bin"
if (-not $Env:PATH.Contains($chocoPath)) {
    $Env:PATH = "$chocoPath$([System.IO.Path]::PathSeparator)$Env:PATH"
    Write-Verbose "Added Chocolatey path to PATH: $chocoPath"
}

# Add Make path to PATH (if it's not already there)
$makePath = "C:\Program Files\GnuWin32\bin"  # Change this if your Make is installed elsewhere
if (-not $Env:PATH.Contains($makePath)) {
    $Env:PATH = "$makePath$([System.IO.Path]::PathSeparator)$Env:PATH"
    Write-Verbose "Added Make path to PATH: $makePath"
}
